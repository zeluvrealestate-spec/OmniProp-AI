...
def run_cycle():
    markets = ["Dubai apartment for sale price 2026", "Riyadh villa for sale price", "Miami condo for sale price Brickell", "London Canary Wharf apartment for sale", "Tokyo Minato ward apartment for sale", "Singapore Orchard condo for sale", "New York Manhattan condo for sale", "Berlin Mitte apartment for sale", "Sydney Harbourfront apartment for sale", "Paris Le Marais apartment for sale"]
    ...
    for market in markets:
        ...
        try:
            ...
            if financials:
                print(f"🧮 [Math] ROI: {financials['annual_roi_pct']}% | Cap Rate: {financials['cap_rate']}%")
            else:
            ...
            print(f"📝 [Agent E] Report generated.")
            
            agent_h = run_groq_agent(PROMPT_H, f"Market:\n{json.dumps(agent_a)}\nProp:\n{json.dumps(agent_g)}\nFin:\n{json.dumps(financials)}\nLegal:\n{json.dumps(agent_c)}")
            print(f"📈 [Agent H] SWOT & Forecast generated.")
            ...
            slack_payload = {
                ...
                "purchase_price": financials['purchase_price'], "monthly_rent": financials['monthly_rent'],
                ...
                "strengths": agent_h.get('strengths', 'N/A'),
                "weaknesses": agent_h.get('weaknesses', 'N/A'),
                "opportunities": agent_h.get('opportunities', 'N/A'),
                "threats": agent_h.get('threats', 'N/A'),
                "forecast_1y": agent_h.get('forecast_1y', 'N/A'),
                "forecast_3y": agent_h.get('forecast_3y', 'N/A'),
                "forecast_5y": agent_h.get('forecast_5y', 'N/A')
            }
...
if __name__ == "__main__":
    print("--- ZELUV IO GLOBAL CLOUD WORKER STARTED ---")
    try:
        run_cycle()
